package com.dincertekin.simpleskyblock.objects.schematics;
...
public class SchematicManager {
    ...
    public void load(String name, double x, double y, double z) {
        try {
            File file = new File(SimpleSkyblock.getPlugin(SimpleSkyblock.class).getDataFolder() + "/schematics/" + name + ".schem");
            ClipboardFormat format = ClipboardFormats.findByFile(file);
            ClipboardReader reader = format.getReader(Files.newInputStream(file.toPath()));
            Clipboard clipboard = reader.read();
            try (EditSession editSession = WorldEdit.getInstance().getEditSessionFactory().getEditSession(BukkitAdapter.adapt(Objects.requireNonNull(Bukkit.getWorld((String) SimpleSkyblock.getPlugin(SimpleSkyblock.class).getConfig().get("general.world")))), -1)) {
